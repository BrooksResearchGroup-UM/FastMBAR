...
class FastMBARSolver:
    def __init__(self, temp, traj_bias_cv, kB, bias_param, bias_func='umbrella', unbiased_energy=None):
        # initialize
        # reshape for 1d CV
        for i in range(len(traj_bias_cv)):
            if traj_bias_cv[i].ndim == 1:
                traj_bias_cv[i] = traj_bias_cv[i].reshape(-1, 1)
    ...
    def computePMF(self, traj_target_cv, target_temp, bins):
        ...
        for i in range(len(traj_target_cv)):
            # reshape for 1d CV
            if traj_target_cv[i].ndim == 1:
                traj_target_cv[i] = traj_target_cv[i].reshape(-1, 1)
        ...
        # the extended coordinate (i.e. self.ext_centers) is of shape (self.L, self.Y.shape[1])
        # we only consider self.Y.shape[1] <= 3
        if self.Y.shape[1] == 1:
            ext_centers = []
            for c1 in self.centers[0]:
                ext_centers.append([c1])
