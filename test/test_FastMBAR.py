...
diff_cpu = np.sqrt(np.mean((mbar.F-ref_result)**2))
print("RMSD (CPU calculation and reference results): {:.2f}".format(diff_cpu))
assert(diff_cpu <= 0.05)
exit()
...
energy_nz = energy[0:num_states_nz, :]
energy_zero = energy[num_sample_nz:, :]
...
## calcuate free energies using CPUs
mbar = FastMBAR(energy_nz, num_conf, bootstrap = False)
mbar._solve()
F_zero, _ = mbar._calculate_free_energies_of_perturbed_states_cpu(energy_zero)
...
F_cpu, F_cpu_std = mbar.calculate_free_energies(bootstrap = True)
