...
# we also test if 1d PMF can be computed properly
# so basically, can keep using fastmbarsolver1, but use new settings for computePMF 
# compute psi and phi 1d PMF, respectively
traj_psi, traj_phi = [], []
for each in traj_bias_cv:
    traj_psi.append(each[:, 0])
    traj_phi.append(each[:, 1])
# compute PMF for psi
theta_bins = np.linspace(-math.pi, math.pi, 50) # use more dense bins
fastmbarsolver1.computePMF(traj_psi, target_temp, [theta_bins])
fastmbarsolver1.writePMF('test-dialanine/pmf-psi.txt')
x1 = fastmbarsolver1.output[:, :1]
f1 = fastmbarsolver1.output[:, 1]
# compute PMF for phi
fastmbarsolver1.computePMF(traj_phi, target_temp, [theta_bins])
fastmbarsolver1.writePMF('test-dialanine/pmf-phi.txt')
x2 = fastmbarsolver1.output[:, :1]
f2 = fastmbarsolver1.output[:, 1]

plt.plot(x1, f1, linewidth=1, label=r'$\psi$')
plt.plot(x2, f2, linewidth=1, label=r'$\phi$')
plt.xlabel(r'$\theta$')
plt.legend()
plt.ylabel(r'Free energy (k_B T)')
plt.savefig('pictures/pmf_1d.pdf')
plt.close()
